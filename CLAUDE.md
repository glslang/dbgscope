...
- **Ending a session is three different things, decided by the opener.** `end_session` (and `Drop`, which mirrors it) ends passively for a dump, a trace and a process this engine *launched* — but a **live kernel** is resumed and actively detached, or it stays frozen at its last break with one CPU halted and the rest spinning, and a process this engine **attached** to is actively detached too, or the kernel kills it: a passive end disconnects the client without detaching, leaving the process marked as being debugged, and `DebugSetProcessKillOnExit` defaults to true, so the debuggee goes when the debugger's process exits. That second exception is newer than the first and had to be recorded rather than asked for — `GetDebuggeeType` answers `DEBUG_CLASS_USER_WINDOWS` / `DEBUG_USER_WINDOWS_PROCESS` for a launch and an attach alike, so `attach_process_begin` sets `attached_live_process` and nothing can derive it afterwards. The detach **falls back to the passive end and still reports the error**, because this teardown is what a client's disconnect runs: a session that will not close is worse than a debuggee that was killed, and a caller told "released" would never go and look. `bc *` first, or an `int3` this engine patched in stays patched in a process that goes on running.
