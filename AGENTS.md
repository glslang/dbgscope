...
- `cargo run --example decode_against_rendering -- <dump> <module>`: cross-check the instruction
  decoder against the engine's own rendering over a whole image. Needs the engine DLLs beside the
  example, as `breakpoint_probe` does.
