...
- **A live `wait()` is a pump, not a wait** (`Arrival`, `DebugEngine::wait_for_live_target`), because **one `WaitForEvent` is one *event*, and not necessarily this target's**. The spawn a `CreateProcessWide` defers is realised inside that wait — but so is everything else the session is holding, so an engine that already has a target can return from it on *that* target's event and leave the open reporting success with its own process absent. Measured (dbgscope#128): an `AttachProcess` break-in whose injected thread is slow to be scheduled arrives a whole wait late, and the `launch_process` after it spends its only wait on that break. Five things to know before editing it. **Membership is not the terminal condition, the last event is** — `cpr` is an ignored filter, so the engine registers a process when it processes that process's create event and carries on, and the initial breakpoint arrives later; a competing break in between leaves this open's process *listed* and not yet where the open promised to leave it, so `presence_of` asks `GetLastEventInformation` whose process it stopped on. That id is the **engine** id and not the pid — measured against `session_processes`, which answers both, and pinned by `test_the_last_event_names_its_process_by_engine_id`, because comparing the wrong one fails silently in the worst direction: nothing ever matches, and every live open pumps to its bound and reports a timeout on a target sitting in front of it. **A launch is identified by elimination and an attach by name** — `CreateProcessWide` hands back no pid, so a launch waits for a process the session did not hold when the launch was issued *and* that this engine did not attach to; exact for opens made one after another, and two launches pending *at once* cannot be told apart by it. **Every failure to ask is `Presence::Unknown`, never `Absent`**, and `Unknown` means opposite things either side of the first wait: before it, the ordinary state of an engine whose target has not materialised, so the wait must still happen; after it, a session there is nothing left to ask, where pumping is what faults. **`LIVE_WAIT_MS` bounds the open, not each wait.** And **asking before the first wait is measured rather than assumed** — neither opener lists its process before the wait that completes it, so the ordinary open still waits exactly once, and what the ask buys is a guard waited *after* something else pumped its target in (29.36s and `E_UNEXPECTED` before, 8.6µs after). `examples/deferred_arrival.rs` is the measurement, including the CPU load that reproduces the race: 3 short in 40 rounds before, 0 in 40 after, 0 in 40 either way on a quiet machine. Its arm G asks the narrower question — whether the wait ended on the launch's *own* event — and answers 0 in 40, which is **not** evidence the window is unreachable: this fixture holds one competing break (the attach's injected break-in), and it is spent on the wider window before the narrower one opens. The condition is tightened on the mechanism, not on that count.
